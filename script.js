// Chave para indentificar os dados salvos pela nossa aplicação no navegador.
...
elements.list.addEventListener("click", function (event) {
  ...
  if (event.target.closest("[data-action='select']")) {
    ...
    const promt = state.prompts.find((p) => p.id === id)

    if (promt) {
      elements.promptTitle.textContent = promt.title
      elements.promptContent.innerHTML = promt.content
      updateAllEditableStates()
  }
}
